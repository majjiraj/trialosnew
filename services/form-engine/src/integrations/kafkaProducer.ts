...
const kafka = new Kafka({ clientId: 'form-engine', brokers: config.kafkaBrokers });
...
export async function connectKafka() {
  try {
    ...
    connected = true;
    console.log('[form-engine] Kafka producer connected');
  } catch (err) {
    console.warn('[form-engine] Kafka unavailable:', err);
  }
}
...
export async function emitEvent(eventType: string, orgId: string, payload: Record<string, unknown>, studyId?: string, actorId?: string) {
  ...
  await producer.send({ topic: 'trialo.events.forms', messages: [{ key: orgId, value: JSON.stringify(message) }] });
}
